...
def run_shell(path: str = None, command: str = None) -> list[str]:
    ...
    For ansible-playbook, forcibly inject our Python (sys.executable) via both env prefix and -e var.
    """
    logs = []
    env = os.environ.copy()
    ...
    try:
        ...
        logs.append(f"⚠️ Warning: current Python ({sys.executable}) is missing boto3/botocore: {e}")

    try:
        if command:
            base_cmd = command.strip()
            # If ansible-playbook, inject interpreter overrides
            ...
            result = subprocess.run(
                ...
                capture_output=True,
                text=True,
                env=env,   # env inherits from app process
            )
            ...
            try:
                ...
                if re.search(r'(^|\s)ansible-playbook(\s|$)', head):
                    # wrap execution: export and pass -e; easiest way is to execute a tiny inline wrapper
